...
\subsection{Pest control and models of host-parasitoid dynamics}
Sur l'utilité des modèles en pest control en général : est-ce que c utile de parler des possibilités en aval par exemple de construire des modèles non autonomes type Jules et la droso, ou type Murdoch 2005 qui prédisent précisément, pr des milieux et des espèces particulières, les outbreaks d’années en années ? 

\subsection{The implicit assumptions on maturation variability of stage-structured models in population dynamics}

\tabto{1 cm}Since many agricultural pests are insects with discontinuous development, stage-structured (or class-structured models) models have been increasingly used to aggregate individuals having certain characteristics, for example, into age classes or size classes. 

\tabto{1 cm}The main classes of stage-structured models make different assumptions about stage transitions. Matrix models treat stage changes as a Markov process, in which the probability of surviving and moving from one stage to another (or not changing) depends simply on the current stage (de Valpine 2014). Resulting discrete-time structured Leslie matrix-type models are convenient if the classes have equal duration and if there is a natural discrete-time pulse in the life cycle, such as annual births (Robertson et al. 2018). 

\tabto{1 cm} Gurney, Nisbet and co-workers (Nisbet and Gurney 1982, Gurney et al. 1983) introduced an approach based on delayed differential equations (DDE), allowing for a more realistic representation of insect lifecycles. This approach takes into account the fact that, while some processes (such as moulting) are discrete, others, such as parasitoid attacks, are more continuous in nature.

\tabto{1 cm}Though, Robertson and co-workers showed that about half of the stage-structured models found in recent literature use the Ordinary Differential Equations (ODE) framework to account for transition between states. Compared with mathematics-focused papers, biology-focused papers were found to be less likely to involve fixed or distributed delays. In such ODE models, the maturation rate is proportional to the current state of the juvenile stage. At the population level, this can be written in the form $\frac{dx}{dt} = -mx$, m being the maturation rate of the stage and 1/m being the mean maturation time. This implicitly assumes an underlying stochastic state transmission model with an exponentially distributed dwell-time in the stage. Particularly, this assumes that the probability of transition is maximal at the entrance of the stage. In line with the work of Gurney and co-workers in 1983, the remaining models by Robertson et al. majorly used fixed delays (DDE). These models do not account for within-species variability in maturation time.  Despite these fundamental differences between the two main approaches, development time variability can greatly influence models' behaviours. 

\subsection{Influence of developmental time variability on predicted population dynamics}
\tabto{1 cm} Fixed delays were shown to be destabilizing factors of communities dynamics as soon as the 1930s (Bailey 1931, cited in Smith and Mead 1974). Seeking higher biological realism, developmental time variability was subsequently introduced and shown to be an important stabilizing factor of community dynamics, including pest populations (Smith and Mead 1974, Blythe et al 1984, Bellows 1986, Briggs 1993, Abrams and Quince 2005, Xu 2010, de Valpine 2014, Robertson 2018), and notably so with experimental support (Cronin et al 2016). In the case of host-parasitoid models, this variability was considered yet another form of parasitism heterogeneity (or risk aggregation), measuring how variably each host is susceptible to parasitism. Risk aggregation was showed to stabilize populations (Chesson and Murdoch 1984, Hassel 2001ab, Taylor 1993, Xu 2010). 

\tabto{1 cm} However, Wearing et al. 2004 show that models' predictions based on the fixed-development approach are differentially sensitive to variability and in key life stages of insects. The stage to which developmental time variability is added, the studied dynamic and the used set of parameters (and their distance to a pre-existing bifurcation) were shown to influence the effect of modified developmental variability. They found that generation cycles could be replaced by longer period cycles in a model of the Plodia interpunctella–Venturia canescens host–parasitoid system. Coupled with demographic stochasticy, this could result in local extinction. Nakamichi et al. (2008) found similar results on a individual-based model inspired by the one of Wearing et al. 2004. These results suggest that the effect of developmental variability should be studied not only on stability but also on persistence, as well as population equilibria, general properties of oscillations when they exist etc. They also suggest that empirical developmental variability should be assessed beforehand to properly model given host-parasitoid dynamics.  

\subsection{Empirical maturation time distributions and the Linear chain trick (LCT)}
\tabto{1 cm}  Insects, and more generally Arthropods, generally display both intrinsic and exogenous variation in stage duration (de Valpine 2014, Manly 1990, Benson 1973 cited by Wearing 2004, ...). A clear review of the methods used to assess developmental variability was given in de Valpine 2014.  Empirical distributions of maturation time often display a delay were no maturation occurs, a clear mode as well as a right-skewness (SOURCE?). A major challenge in using arbitrary maturation time distributions is that the models become integrodifferential (ID) and quickly lead away from analytical methods towards computational ones (de Valpine 2014). Hence, modelers have relied on a few main distributions. Gamma (or Erlang, a special case where the shape parameter is a positive integer) and Normal distributions have been widely used in distributed development models of insect populations (SOURCE? deValpine, Robertson?, Wearing?). They allow for a sufficiently realistic hypothesis with relatively few parameters associated. 

\tabto{1 cm} It is however possible to account for more realistic stage transition while also construct models that allow for relatively simple algebraic analysis. Erlang distributions can implicitly be obtained by dividing each stage into n sub-stages governed by a system of n coupled ODEs (MacDonald 1978, Hurtado 2018). This is called the Linear Chain Trick (LCT) and has been increasingly used in ecological (Smith and Mead 1974, McNair 1995, Wearing 2004, Abrams and Quince 2005, Bjornstad et al. 2016, Nelson et al. 2019) and epidemiological studies (Wearing 2005, Hurtado 2019). Hurtado and Kirosingh (2019) suggested a method to bypass the ID framework and allow modelers to assume maturation (or "passage") time distributions from a family called the phase-type distributions. These distributions can be thought as the family of all possible hitting time distributions for continuous-time Markov chains. In the case of a "linear chain" of ODEs, the Erlang distribution is involved. As suggested by the example SEIR model with heterogeneity among infected individuals showed by Hurtado and Cameron (2021), this new framework allows highly flexible ODE models assuming more realistic maturation time distribution and can be computationally more efficient. Even outside this framework, the LCT can be judicious. Nelson et al. (2019) used the LCT to implement hierarchical asymetric larval competition : they assumed the per-capita mortality rate to depend only on the density of animals that are larger or equal to the size of each individual. The competition mortality terms uniquely depend on the downstream LCT subcompartiments. 

\tabto{1 cm} Xu et al. 2010 studied the sharpe effect of the use of the shifted gamma maturation time distribution on the well-known host-parasitoid built by Godfray et al. (1987, 1989). They also surveyed the Gamma (or Erlang) shape parameter (n) that can be estimated from the literature of laboratory insect development. We would like to complement this work in two ways. Critically, in line with the promising framework of the (Generalized) LCT (Hurtado and Cameron 2021), we will simply apply the LCT which does not allow to shift the Erlang distribution. Hence, we will reconduct the survey of n with this new constraint. Second, we will study several classical host-parasitoid models. Inspired by the "sensitivity" concept suggested by Wearing (2004), we will apply the LCT to several models and assess the robustness of their various predictions/properties (lequel choisir?) to the change of n.
...
\centering
	\begin{subfigure}[t]{0.48\textwidth}
	  \centering
	  \includegraphics[width=\textwidth]{/home/mathisdr/Desktop/2024-2025/Stage/Jupyter_stage/Figures/n_survey_not_clipped.pdf}
	  \caption{}
	  \label{fig:a}
	\end{subfigure}
	\hfill
	...
	\begin{subfigure}[t]{0.48\textwidth}
	  \centering
	  \includegraphics[width=\textwidth]{/home/mathisdr/Desktop/2024-2025/Stage/Jupyter_stage/Figures/n_survey_clipped.pdf}
	  \caption{}
	  \label{fig:b}
	\end{subfigure}
...
\vspace{1em} % Optional: vertical space between rows
	...
	\begin{subfigure}[t]{0.5\textwidth}
	  \centering
	  \includegraphics[width=\textwidth]{/home/mathisdr/Desktop/2024-2025/Stage/Jupyter_stage/Figures/n_survey_some_species.pdf}
	  \caption{}
	  \label{fig:c}
	\end{subfigure}
