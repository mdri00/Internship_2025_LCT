...
Effet de n sur $p_{survie}$ (cf oral de Juin)
\\
Ressemblance frappante entre l'effet de l'agrégation spatiale du parasitisme (ajout de pseudointerférénce via k dans un modèle à délai fixe) et l'effet de l'agrégation temporelle du parasitisme (ajout de variab dev. via n dans un modèle sans pseudointerférence)
...
\tabto{1.5 cm} -> Let sensitivity (understand : to the increase of n) be the propensity of ODE models’ prediction to swiftly change as soon as maturation time variability is reduced (as long as n increases from 1).  Note that this definition of sensitivity is inverse to the one chosen by Wearing et al. In any case, they showed that sensitivity was variable and dependent on several factors (to which developmental time variability is added ; to which dynamic is studied ; to the used set of parameters and their distance to a pre-existing bifurcation).  
\\
